...
export function CategoryCard({ category, large = false }: CategoryCardProps) {
  return (
    <Link
      ...
      className="group relative flex h-full overflow-hidden rounded-2xl bg-subtle"
    >
      {/* Text — top left */}
      <div className="absolute top-6 left-6 z-10 md:top-8 md:left-8">
        ...
        <span className="inline-flex items-center gap-1 mt-2 text-sm text-primary border-b border-primary pb-px group-hover:text-accent group-hover:border-accent transition-colors">
          Shop Now →
        </span>
      </div>
