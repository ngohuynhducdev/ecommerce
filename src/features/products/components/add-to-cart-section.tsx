...
export function AddToCartSection({ product }: AddToCartSectionProps) {
  ...
  return (
    <div className="mt-6">
      {/* 1. Countdown timer */}
      <div>
        <p className="text-sm text-subtle mb-3">Offer expires in:</p>
        <div className="flex gap-3">
          {timerUnits.map(({ label, value }) => (
            <div key={label} className="flex flex-col items-center gap-1">
              <div className="w-14 h-14 bg-mist rounded-lg flex items-center justify-center text-xl font-semibold tabular-nums text-ink">
      ...
      <div>
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-ink">Measurements</span>
          <span className="text-sm text-subtle">/</span>
        </div>
        <p className="text-sm text-subtle mt-1">17 1/2×20 5/8&quot;</p>
      </div>
      ...
      {colorVariants.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center">
            <span className="text-sm font-medium text-ink">Choose Color</span>
            <span className="text-subtle ml-1">›</span>
          </div>
          {selectedColor && (
            <p className="text-sm text-subtle capitalize mt-0.5">{selectedColor.value}</p>
          )}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    hasBorder ? "border-line" : "border-transparent",
      ...
      {sizeVariants.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2">
            <p className="font-medium text-sm text-ink">Size:</p>
            <button className="text-xs text-subtle hover:text-ink underline cursor-pointer">
              Size Guide
            </button>
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isUnavailable && "opacity-40 cursor-not-allowed hover:border-line"
                  )}
                >
                  {variant.value}
                </button>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <div className="flex items-center border border-ink rounded-sm">
          <button
            ...
            disabled={quantity <= 1}
            className="w-10 h-10 flex items-center justify-center text-ink hover:bg-mist transition-colors disabled:opacity-40 cursor-pointer"
        ...
        <button
          ...
          className="flex-1 border border-ink rounded-sm flex items-center justify-center gap-2 text-sm font-medium text-ink hover:bg-mist transition-colors cursor-pointer"
        >
          <svg
            width="16"
            height="16"
      ...
      <button
        ...
        className="w-full h-14 bg-ink text-white text-sm font-medium mt-3 rounded-sm hover:bg-gold transition-colors disabled:opacity-40 cursor-pointer"
      >
      ...
      <div className="mt-6 pt-4 border-t border-line space-y-2">
        <div className="flex items-center gap-8">
          <span className="text-xs font-semibold uppercase tracking-widest text-subtle w-20">SKU</span>
          <span className="text-xs text-ink">{product.id.slice(0, 6)}</span>
        </div>
        <div className="flex items-center gap-8">
          <span className="text-xs font-semibold uppercase tracking-widest text-subtle w-20">Category</span>
          <span className="text-xs text-ink">{product.category.name}</span>
        </div>
      </div>
    </div>
  );
