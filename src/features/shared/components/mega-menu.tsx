...
export function MegaMenu({ onMouseEnter, onMouseLeave }: MegaMenuProps) {
  return (
    <div
      ...
      className="absolute top-full left-0 w-full bg-white shadow-lg border-t border-line z-50 p-10"
    >
        ...
        {categories.map((cat) => (
          <div key={cat.slug}>
            <Link
              ...
              className="text-sm font-semibold text-ink mb-4 block hover:text-gold transition-colors"
            >
              {cat.title}
            </Link>
            <ul className="space-y-1">
              {cat.links.map((link) => (
                <li key={link}>
                  <Link
                    href={`/shop?category=${cat.slug}`}
                    className="text-sm text-subtle hover:text-ink py-1 block transition-colors"
                  >
                    {link}
                  </Link>
                </li>
