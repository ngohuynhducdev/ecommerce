    ...
    <section className="py-12 px-8 lg:px-20 bg-white">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {features.map((feature) => (
          <div key={feature.label} className="bg-subtle rounded-2xl p-8 flex flex-col">
            <span className="text-primary mb-6">{feature.icon}</span>
            <p className="font-semibold text-base text-primary">{feature.label}</p>
            <p className="text-sm text-muted mt-1.5">{feature.description}</p>
          </div>
        ))}
      </div>
    </section>
