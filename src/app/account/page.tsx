...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="block text-xs font-medium text-ink uppercase tracking-wide mb-2">
      {children}
    </label>
  );
}
...
const inputClass =
  "w-full h-12 border border-line rounded-lg px-4 text-sm outline-none focus:border-ink transition-colors placeholder:text-subtle";
...
export default function AccountPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-xl">
      <h2 className="text-xl font-semibold text-ink mb-6">Account Details</h2>
      ...
      <h2 className="text-xl font-semibold text-ink mt-10 mb-6">Password</h2>
      ...
      <button
        type="submit"
        className="mt-8 h-12 px-8 bg-ink text-white text-sm font-medium rounded-lg hover:bg-gold transition-colors cursor-pointer"
      >
        Save changes
      </button>
    </form>
