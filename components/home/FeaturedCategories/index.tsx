...
export async function FeaturedCategories() {
  ...
  try {
    ...
    // Strapi not running — show placeholder grid
  }
  ...
  const placeholders = [
    ...
    { id: "p2", name: "Phòng ngủ", slug: "phong-ngu", color: "bg-sage-200" },
    { id: "p3", name: "Phòng ăn", slug: "phong-an", color: "bg-earth-200" },
    { id: "p4", name: "Văn phòng", slug: "van-phong", color: "bg-warm-300" },
  ];
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {categories.length > 0
          ? categories.map((cat) => {
              const imageUrl = cat.attributes.image?.data?.attributes?.url
          ...
          : placeholders.map((p) => (
              <Link
                ...
                className={`group relative aspect-[3/4] rounded-2xl overflow-hidden ${p.color} shadow-sm hover:shadow-md transition-shadow`}
              >
